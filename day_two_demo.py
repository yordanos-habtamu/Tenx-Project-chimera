...
async def demonstrate_system():
    ...
    moderation_result = await moderation_agent.process_task(moderation_task)  # Using process_task for proper wrapper
    print(f"✅ Moderation completed - Approved: {moderation_result['result']['approved']}")
    ...
    publish_result = await publisher_agent.process_task(publish_task)  # Using process_task for proper wrapper
    print(f"📤 Attempted publishing to {len(publish_result['result']['publish_results'])} platforms")
    print(f"✅ Successful publishes: {publish_result['result']['successful_publishes']}")
    print()
    ...
    for agent_id, status in statuses.items():
        print(f"  {status['name']}: {status['status']} (updated: {status['last_updated'][-9:-1]})")
    print()
    ...
    print("✅ ContentSwarm with ScriptWriter, VideoGenerator, and ThumbnailDesigner agents")
