...
class TestSkillFetchTrendsInterface:
    ...
    def test_output_structure(self):
        ...
        assert all(
            key in mock_output["metadata"]
            for key in ["platform", "fetched_at", "region"]
        )
...
class TestSkillGenerateScriptInterface:
    ...
    def test_input_validation(self):
        ...
        assert all(
            key in valid_input["trend"] for key in ["keyword", "context", "volume"]
        )
...
class TestSkillGenerateVideoInterface:
    ...
    def test_input_validation(self):
        ...
        assert all(
            key in valid_input["script"] for key in ["title", "body", "duration"]
        )
...
class TestSkillPublishContentInterface:
    ...
    def test_output_structure(self):
        ...
        assert (
            metadata["successful"] + metadata["failed"] == metadata["total_platforms"]
        )
...
async def test_skills_can_be_composed():
    ...
    pipeline_data = {
        "trends_input": {"platform": "google_trends", "region": "US", "limit": 5},
