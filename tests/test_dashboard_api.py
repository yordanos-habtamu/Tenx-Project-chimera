...
def mock_orchestrator():
    ...
    orchestrator.agents["trend_fetcher_001"] = mock_agent # Add default agent for manual trigger test
...
def test_control_agent_success(override_dependency):
    ...
    # Test Pause
    response = client.post("/api/v1/dashboard/agents/test_agent_001/control?action=pause")
    assert response.status_code == 200
    data = response.json()
...
def test_control_agent_invalid_action(override_dependency):
    """Test invalid action"""
    response = client.post("/api/v1/dashboard/agents/test_agent_001/control?action=dance")
    assert response.status_code == 400

def test_control_agent_not_found(override_dependency):
    """Test controlling non-existent agent"""
    response = client.post("/api/v1/dashboard/agents/missing_agent/control?action=start")
