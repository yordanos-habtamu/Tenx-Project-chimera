...
class ContentService:
    ...
    async def create_content_from_research(
        self, research_data: dict[str, Any], content_type: str = "educational"
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            video_record = await self._save_video_record(
                {**script_result, **video_result, **thumbnail_result}
            )
            ...
            logger.info(
                f"Content creation workflow completed successfully: {video_record.id}"
            )
    ...
    async def _generate_script(
        self, research_data: dict[str, Any], content_type: str
    ) -> dict[str, Any]:
        """
    ...
    def get_content_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "latest_workflow": (
                self.workflow_history[-1] if self.workflow_history else None
            ),
        }
