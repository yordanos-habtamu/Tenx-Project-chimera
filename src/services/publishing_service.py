...
class PublishingService:
    ...
    async def _publish_to_platform(
        self, content_data: dict[str, Any], platform: str, schedule_immediate: bool
    ) -> dict[str, Any]:
        """
        ...
        if success:
            ...
            url = platform_urls.get(
                platform, f"https://{platform}.com/content/{content_id}"
            )
    ...
    def _validate_content_for_platform(
        ...
        if platform == "youtube" and len(script) > config.get(
            "max_description_length", 5000
        ):
            errors.append(f"Description too long for {platform}")
        elif platform == "twitter" and len(script) > config.get(
            "max_tweet_length", 280
        ):
            errors.append(f"Content too long for {platform}")
        elif platform == "instagram" and len(script) > config.get(
            "caption_max_length", 2200
        ):
            errors.append(f"Caption too long for {platform}")
        elif platform == "tiktok" and len(script) > config.get(
            "max_caption_length", 150
        ):
            errors.append(f"Caption too long for {platform}")
        ...
        if platform == "tiktok":
            ...
            if duration > max_duration:
                errors.append(
                    f"Video too long for {platform}: {duration}s (max: {max_duration}s)"
                )
    ...
    async def bulk_publish(
        self, content_list: list[dict[str, Any]], platforms: list[str]
    ) -> dict[str, Any]:
        """
    ...
    def get_publishing_statistics(self) -> dict[str, Any]:
        ...
        total_attempts = sum(
            pub.get("total_platforms_attempted", 0) for pub in self.publication_history
        )
        total_successes = sum(
            pub.get("successful_publishes", 0) for pub in self.publication_history
        )
        total_failures = sum(
            pub.get("failed_publishes", 0) for pub in self.publication_history
        )
        ...
        return {
            ...
            "latest_publication": (
                self.publication_history[-1] if self.publication_history else None
            ),
        }
