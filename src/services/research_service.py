...
class ResearchService:
    ...
    def __init__(self):
        ...
        self.niche_categories = [
            ...
            "health_fitness", "cooking", "travel", "DIY_crafts", "science",
            "art_culture", "business", "sports"
        ]
        self.research_cache = {}
    ...
    async def analyze_trends(
        ...
        # Generate trend data
        trends = await self._fetch_trends(keywords or [topic] if topic else ["AI", "Technology"])
    ...
    async def _analyze_trend_patterns(self, trends: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        insights = []
        if rising_trends:
            insights.append(f"Found {len(rising_trends)} rising trends worth considering")
        if high_volume_trends:
            insights.append(f"Found {len(high_volume_trends)} high-volume opportunities")
        ...
        return {
            ...
            "insights": insights,
            "trend_diversity": len(set(t["keyword"] for t in trends))
        }
        ...
        for category in self.niche_categories:
            ...
            for keyword in keywords:
                ...
                if keyword_lower in category_lower or category_lower in keyword_lower:
                    ...
                    relevance_score += 0.5
                elif keyword_lower in ["ai", "machine learning", "technology"] and "tech" in category_lower:
                    relevance_score += 0.7
                elif keyword_lower in ["education", "learning", "tutorial"] and "educational" in category_lower:
                    relevance_score += 0.7
                elif keyword_lower in ["finance", "money", "investing"] and "finance" in category_lower:
                    relevance_score += 0.7
                elif keyword_lower in ["health", "fitness", "wellness"] and "health" in category_lower:
                    relevance_score += 0.7
            ...
            if relevance_score > 0:
                niche_data = {
                    ...
                    "estimated_engagement_rate": round(random.uniform(1.0, 12.0), 2),  # Percentage
    ...
    def _get_content_types_for_niche(self, niche_category: str) -> List[str]:
        ...
        """
        content_mapping = {
            "tech_reviews": ["product_reviews", "tutorials", "unboxings", "comparisons"],
            "educational_content": ["tutorials", "explainer_videos", "course_content", "how-tos"],
            ...
            "cooking": ["recipe_videos", "cooking_tips", "ingredient_reviews", "techniques"],
            "travel": ["destination_reviews", "travel_tips", "vlog", "guides"],
            "DIY_crafts": ["tutorials", "project_demonstrations", "supply_reviews", "ideas"],
            "science": ["experiments", "explainer_videos", "discoveries", "research_summaries"],
            "art_culture": ["tutorials", "reviews", "historical_content", "technique_explanations"],
            "business": ["advice", "case_studies", "trends", "strategy"],
            "sports": ["analysis", "highlights", "training_tips", "reviews"]
        }
        
        return content_mapping.get(niche_category, ["general_content", "interviews", "discussions"])
    ...
    def get_research_statistics(self) -> Dict[str, Any]:
        ...
        trend_analyses = sum(1 for analysis in self.analysis_history 
                           if "fetched_trends" in analysis)
        niche_analyses = sum(1 for analysis in self.analysis_history 
                           if "identified_niches" in analysis)
    ...
    async def _generate_recommendations(
        ...
        rising_trends = [t for t in trend_analysis.get("fetched_trends", []) 
        ...
        low_competition_niches = [n for n in niche_analysis.get("identified_niches", []) 
                                if n.get("competition_level") == "low"]
        if low_competition_niches:
            top_low_comp = max(low_competition_niches, key=lambda x: x["relevance_score"])
            recommendations.append({
