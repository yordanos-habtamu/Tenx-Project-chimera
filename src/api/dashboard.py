...
async def get_system_config():
    ...
    safe_config = settings.model_dump(
        exclude={"secret_key", "openrouter_api_key", "video_gen_api_key"}
    )
    return {
...
async def control_agent(agent_id: str, action: str, request: Request):
    ...
    if action == "pause":
        ...
        agent.status = (
            AgentStatus.IDLE
        )  # 'Stop' often means reset to IDLE in soft-systems

    logger.info(
        f"Agent {agent_id} control action '{action}' executed. Status: {previous_status} -> {agent.status}"
    )
...
async def trigger_manual_task(
    task_type: str, payload: dict[str, Any], request: Request
):
    """
    ...
    if not target_agent_id:
        raise HTTPException(
            status_code=400, detail=f"Unknown or router-less task type: {task_type}"
        )
    ...
    try:
        if target_agent_id in orchestrator.agents:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"Target agent {target_agent_id} not found/active",
            )
    except Exception as e:
