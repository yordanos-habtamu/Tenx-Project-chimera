from ..core.base_agent import BaseAgent, AgentOrchestrator
...
class SupervisorAgent(BaseAgent):
    ...
    async def coordinate_content_creation(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        distribution_results = await self._execute_distribution_phase(validation_results)
    ...
    async def _execute_research_phase(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        research_agents = [aid for aid in self.swarms["research"]]
        ...
        results = await self.orchestrator.broadcast_task(
            research_task,
            lambda agent: agent.agent_id in research_agents
        )
        
        return {"results": results, "phase": "research"}
    
    async def _execute_content_phase(self, research_results: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute the content generation phase using ContentSwarm agents.
        """
        logger.info("Executing content phase...")
        
        content_agents = [aid for aid in self.swarms["content"]]
        ...
        results = await self.orchestrator.broadcast_task(
            content_task,
            lambda agent: agent.agent_id in content_agents
        )
        
        return {"results": results, "phase": "content"}
    
    async def _execute_safety_phase(self, content_results: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute the safety/validation phase using SafetyLayer agents.
        """
        logger.info("Executing safety phase...")
        
        safety_agents = [aid for aid in self.swarms["safety"]]
        
        if not safety_agents:
            logger.warning("No safety agents registered")
            return {"error": "No safety agents available", "content_passed_validation": False}
        ...
        results = await self.orchestrator.broadcast_task(
            validation_task,
            lambda agent: agent.agent_id in safety_agents
        )
        
        # Check if content passed validation
        content_passed = all(r.get("result", {}).get("approved", False) for r in results)
    ...
    async def _execute_distribution_phase(self, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if not validation_results.get("content_passed_validation", False):
            logger.warning("Content did not pass safety validation, skipping distribution")
            return {"skipped": True, "reason": "Failed safety validation"}
        
        distribution_agents = [aid for aid in self.swarms["distribution"]]
        ...
        results = await self.orchestrator.broadcast_task(
            distribution_task,
            lambda agent: agent.agent_id in distribution_agents
        )
    ...
    async def publish_content(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        validation_results = await self._execute_safety_phase({"content_data": task_data})
