...
class ScriptWriterAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "script_writer_001", name: str = "ScriptWriterAgent"):
        ...
        if OPENROUTER_API_KEY and AsyncOpenAI:
            ...
            logger.warning("OpenRouter API Key not found or openai not installed. Falling back to mock generation.")
    ...
    async def write_script(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        primary_trend = max(trends, key=lambda x: x.get("volume", 0)) if trends else trends[0]
        ...
        if self.client:
            try:
                ...
                logger.error(f"OpenRouter generation failed: {e}. Falling back to template.")
                
        # Fallback to template if no client or error
        return self._generate_with_template(topic, content_type, primary_trend)

    async def _generate_with_llm(self, topic: str, content_type: str, trend_data: Dict) -> Dict[str, Any]:
        ...
        response = await self.client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a professional viral video script writer."},
    ...
    def _generate_with_template(self, topic: str, content_type: str, trend_data: Dict) -> Dict[str, Any]:
...
class VideoGeneratorAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "video_gen_001", name: str = "VideoGeneratorAgent"):
        super().__init__(agent_id, name)
    ...
    async def _generate_with_api(self, title: str, script: str, video_id: str) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"{VIDEO_GEN_ENDPOINT}/jobs", json=payload, headers=headers) as resp:
                if resp.status != 200:
            ...
            while (datetime.utcnow() - start_time).seconds < CONTENT_GENERATION_TIMEOUT:
                ...
                async with session.get(f"{VIDEO_GEN_ENDPOINT}/jobs/{job_id}", headers=headers) as resp:
                    if resp.status != 200:
                        continue
                    ...
                    if job_status.get("status") == "completed":
                        ...
                        raise Exception(f"Video generation failed: {job_status.get('error')}")
...
class ThumbnailDesignerAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "thumbnail_designer_001", name: str = "ThumbnailDesignerAgent"):
        super().__init__(agent_id, name)
        self.color_schemes = [
    ...
    async def design_thumbnail(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        content_data = task_data.get("content_data", {})
        script_data = content_data.get("results", [{}])[0].get("result", {}) if isinstance(content_data, dict) else content_data
        ...
        text_elements = [
            {"text": trend_keyword.upper(), "position": "top", "size": "large", "color": color_scheme[0]},
